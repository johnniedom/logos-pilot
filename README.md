...
| `agents/owner-channel.sh` | The owner from separate apps | Agent + two owner front-ends that share nothing with it but a relay, both built from one library: the `pilot_owner` module (what the Basecamp plugin Pilot Remote calls; driven headlessly in a second daemon) makes the owner key, pairs, gets `/balance`, approves a held 101-LEZ spend settled on chain; the `pilot-owner` console client imports the same key, gets `/balance`, sends 20 LEZ under the limit, settled on chain (`owner-channel.yml`; see `docs/owner-guide.md`) |
| `agents/local-owner-demo.sh` | Basecamp talks to an agent through the relay, on one machine | Starts a relay, deploys an agent bound to your owner key, installs the `pilot_owner` module + the Pilot Remote plugin into Basecamp and pairs; you then type `/balance`, `/send … 101`, `/approve <id>` in the Basecamp window |
...
| `install-basecamp.sh` | For GUI | Installs the agent modules, the `pilot_owner` module and the two plugins into Logos Basecamp: Pilot Agent (an agent inside this Basecamp) and Pilot Remote (an agent anywhere else, over the owner channel) |
